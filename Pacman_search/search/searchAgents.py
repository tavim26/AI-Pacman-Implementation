...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        # Start state is the starting position and an empty tuple of visited corners
    ...
    def isGoalState(self, state):
        ...
        """
        # Goal is reached when all corners are visited
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                # Add corner to visited if Pacman reaches it
                if nextPosition in self.corners:
                    nextVisitedCorners.add(nextPosition)
                # Add the successor state with cost of 1
...
def cornersHeuristic(state, problem: CornersProblem):
    ...
    # Greedily calculate the minimum distance to visit all unvisited corners
...
def foodHeuristic(state: Tuple[Tuple[int, int], List[List[bool]]], problem: FoodSearchProblem):
    """
    A heuristic for the FoodSearchProblem.

    state: The current search state, a tuple (pacmanPosition, foodGrid)
    problem: The FoodSearchProblem instance for this layout.
    """
    position, foodGrid = state
    foodList = foodGrid.asList()  # Get a list of all food positions

    # If there is no food left, the heuristic is 0
    if not foodList:
        return 0

    # Calculate the Manhattan distances to all food dots
    distances = [util.manhattanDistance(position, food) for food in foodList]

    # Use the maximum Manhattan distance to estimate the cost
    # This ensures that the heuristic is admissible and non-negative
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        # Creează instanța problemei AnyFoodSearchProblem
        problem = AnyFoodSearchProblem(gameState)

        # Folosește BFS pentru a găsi calea către cel mai apropiat punct de mâncare
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        """
        x, y = state
        # Verificăm dacă există mâncare în această locație
